    ...
    public partial class frmBrowser : Form
    {
        ...
        private void wbAuth_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
                ...
                url == "https://store.steampowered.com//login/transfer")
            {
                ...
                var steamloginid = string.Empty;
                foreach (Cookie cookie in cookies)
                {
                    if (cookie.Name.StartsWith("steamLogin"))
                    {
                ...
                if (Settings.Default.steamLogin != null && Settings.Default.steamLogin.Length > 17)
                    foreach (Cookie cookie in cookies)
                    {
                        if (cookie.Name.Equals(string.Format("steamMachineAuth{0}", Settings.Default.steamLogin.Substring(0, 17))))
                        {
